/**
 * Metric configuration helpers: map between user-facing metric options
 * and the underlying raw metrics returned by the API.
 */
import {
  GCServerMetric,
  GCUnit,
...
/**
 * Central registry that defines how each logical metric should be labeled,
 * what base metric to use from the API and which unit to display.
 */
...
/** Builds `<Select>` option list for the query editor. */
export const createOptions = () =>
  Object.entries(config).map(
    ([value, { label }]) => ({ value, label } as SelectableValue<GCMetric>)
  );
/** Creates a single metric option preserving the config metadata. */
...
export const getUnitByMetric = (metric: GCMetric): GCUnit =>
  config[metric].unit;
/**
 * Returns a function that extracts the raw sample points for the
 * requested logical metric from a response document.
 */
...
/**
 * Returns a function that extracts only the Y values for the requested
 * logical metric. This is useful when computing min/max/aggregations.
 */
